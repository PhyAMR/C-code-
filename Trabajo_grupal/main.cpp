...
auto main() -> int {
    ...
    (std::cin>>N.number()).ignore(10); //Guardamos el valor numérico de lo introducido para usarlo a la hora de hacer funciones

    std::cout<<"Insert intensity of the current in amperes: "; 
    (std::cin>>I.number()).ignore(10);

    std::cout<<"Insert lenght of the solenoid in milimetres: "; 
    (std::cin>>L.number()).ignore(10);

    std::cout<<"Insert diametre of the solenoid in milimetres: "; 
    (std::cin>>D.number()).ignore(10);

    std::cout<<"Insert the path of the CSV file (C:/Users/user_name/Desktop/file_name.csv): "; 
    (std::cin>>Path).ignore(10) ; // Preguntamos al usuario por el camino y lo guardamos en Path
    ...
    while( my_input_file.fail()) // Si el archivo no esta en la ubicación que ha escrito el usuario
{
    ...
    std::cout<<"Incorrect file path, Re-Enter path: "; // Volvemos a pedir que inserte el camino
    std::cin>>Path;
    ...
    /*parameter_t residual_derivative = [&](Data const& d, parameter_t const& param) -> auto {
        ...
        auto const sqrt_one = un::sqrt(un::pow<2>(a) + un::pow<2>(R));
        auto const sqrt_two = un::sqrt(un::pow<2>(b) + un::pow<2>(R));
        auto const first_term = -(un::pow<2>(R))/((un::pow<2>(a) + un::pow<2>(R))*sqrt_one);
        auto const second_term = (un::pow<2>(R))/((un::pow<2>(b) + un::pow<2>(R))*sqrt_two);

        auto i_0 = (mu_0/2)*(N*I/L)*(first_term+second_term);
        auto i_1 = (1/2)*(N*I/L)*(a/sqrt_one - b/sqrt_two);

        auto temp = z_0_param*i_0 + mu_0_param*i_1 ;
...
for(Data const& dt : data_vec){
    Z.push_back(dt.position.number());
}

auto B = std::vector<double>{};
for(Data const& dt : data_vec){
    B.push_back(dt.magnetic_induction.number());
}



auto [Zmin, Zmax] = std::ranges::minmax_element(Z);

mp::gcf()->quiet_mode(mp::on);
mp::xlabel(" z [cm]");
mp::ylabel("B [mT]");
mp::title("Campo magnético de un solenoide");

auto f = [&](double z)-> double {
    return theoretic_magnetic_induction(len_t{z}, z_0, mu_0).number();
};

mp::fplot(f, std::array<double, 2>{*Zmin,*Zmax})-> color("k").line_width(1.5);
mp::hold(mp::on);
mp::stem(Z, B, "filled") -> color("k").line_width(0.25).marker_face_color("w");
mp::show();
